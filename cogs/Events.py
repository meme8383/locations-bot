...
class Kgnar_message: 
    id = 363076151867867144  # Kgnar's id 
    @staticmethod
    ...
    def __init__(self, content): 
        ...
        self.msg = self.content.lower().strip() # 'clean' version of the message to use in checks 
    ...
    def neutral_emoji_check(self): 
        if ':neutral_face:' in self.msg: return "SMH STOP USING THAT EMOJI STUPID STUPID STUPID :neutral_face:"
    def grin_emoji_check(self): 
        if ':grin:' in self.msg: return "YOU ARE NOT FUNNY IDOT STOP SOTP SOPT SPOTSPOTPOSPO"
    ...
    def ew(self): 
        if ":face_with_raised_eyebrow:" in self.msg: return ":face_with_raised_eyebrow:kgnar spoke:face_with_raised_eyebrow:, must get her manager <@360392861608574978>"
    def no(self):
        if "no" and "furry" in self.msg: return "bad furry."
        elif "no" in self.msg: return "bad kgnar. :thumbsdown: :thumbsdown: :thumbsdown: :thumbsdown: :thumbsdown: :thumbsdown: "
    def asked(self):
        if ":rage:" in self.msg: return "nobody asked though :rofl: :rofl: :rofl:"
    def roll(self):
        if "rolling_eyes" in self.msg: return "kgnar is misbehaving again, I apologize for this incident. please contact her manager"
    def cry(self):
        if ":sob:" in self.msg: return ":joy::joy::joy::joy::joy:"
    def yawn(self):
        if ":yawning_face:" in self.msg: return "shut up"
    def swear(self): 
...
class Events(commands.Cog): 
    ...
    async def on_message(self, message): 
        ...
        if message.author.id == Kgnar_message.id: # if we enter here, kgnar has sent a message   
            ...
            message.channel.send("the ultimate furry :nauseated_face:")
